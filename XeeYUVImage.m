#import "XeeYUVImage.h"



static void XeeMakeYCCLookupTable();
static void XeeYUVImageReadPixel(void *datarow,int x,uint8 *pixel,void *context);



@implementation XeeYUVImage

-(id)init
{
	if(self=[super init]) [self _initYUVImage];
	return self;
}

-(id)initWithFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes
{
	if(self=[super initWithFile:name firstBlock:block attributes:attributes]) [self _initYUVImage];
	return self;
}

-(id)initWithWidth:(int)framewidth height:(int)frameheight
{
	if(self=[super init])
	{
		[self _initYUVImage];
		if([self allocWithWidth:framewidth height:frameheight]) return self;
		[self release];
	}

	return nil;
}

-(void)_initYUVImage
{
	freedata=NULL;
}

-(void)dealloc
{
	if(freedata) free(data);

	[super dealloc];
}



-(void)setData:(void *)pixeldata width:(int)framewidth height:(int)frameheight bytesPerRow:(int)bprow
{
	data=pixeldata;
	width=framewidth;
	height=frameheight;
	bytesperrow=bprow;

	texintformat=GL_RGB8;
	texformat=GL_YCBCR_422_APPLE;
	#ifdef BIG_ENDIAN
	textype=GL_UNSIGNED_SHORT_8_8_REV_APPLE;
	#else
	textype=GL_UNSIGNED_SHORT_8_8_REV_APPLE;
	#endif
	pixelsize=2;
}

-(BOOL)allocWithWidth:(int)framewidth height:(int)frameheight
{
	int bprow=(2*framewidth+2)&~3;
	freedata=malloc(frameheight*bprow);

	if(freedata)
	{
		unsigned long *ptr=(unsigned long *)freedata;
		#ifdef BIG_ENDIAN
		for(int i=0;i<frameheight*bprow/4;i++) *ptr++=0x7f007f00;
		#else
		for(int i=0;i<frameheight*bprow/4;i++) *ptr++=0x007f007f;
		#endif

		[self setData:freedata width:framewidth height:frameheight bytesPerRow:bprow];
		return YES;
	}
	return NO;
}

-(void)fixYUVGamma
{
	static const unsigned char gammatable[256]=
	{
		,
	};

	for(int y=0;y<height;y++)
	{
		int n=width;
		unsigned char *ptr=(unsigned char *)data+y*bytesperrow+1;

		while(n--)
		{
			*ptr=gammatable[*ptr];
			ptr+=2;
		}
	}
}



-(int)CGImageBitsPerComponent { return 8; }

-(int)CGImageBitsPerPixel { return 24; }

-(CGBitmapInfo)CGImageBitmapInfo { return 0; }

-(CGColorSpaceRef)CGImageCopyColorSpace { return CGColorSpaceCreateDeviceRGB(); }

-(XeePixelAccessFunc)CGImageReadPixelFunc { return XeeYUVImageReadPixel; };

-(void *)CGImageReadPixelContext { return NULL; };

+(void)load { XeeMakeYCCLookupTable(); }

@end



#define SCALEBITS	16
#define ONE_HALF	((int32)1<<(SCALEBITS-1))
#define FIX(x)		((int32)((x)*(1L<<SCALEBITS)+0.5))

static int cr_r_tab[256],cb_b_tab[256],cr_g_tab[256],cb_g_tab[256];
static uint8 *range_limit,range_table[256*3];

static void XeeMakeYCCLookupTable()
{
	for(int i=0,x=-128;i<=256;i++,x++)
	{
		/* i is the actual input pixel value, in the range 0..MAXJSAMPLE */
		/* The Cb or Cr value we are thinking of is x = i - CENTERJSAMPLE */
		/* Cr=>R value is nearest int to 1.40200 * x */
		cr_r_tab[i]=(FIX(1.40200)*x+ONE_HALF)>>SCALEBITS;
		/* Cb=>B value is nearest int to 1.77200 * x */
		cb_b_tab[i]=(FIX(1.77200)*x+ONE_HALF)>>SCALEBITS;
		/* Cr=>G value is scaled-up -0.71414 * x */
		cr_g_tab[i]=(-FIX(0.71414))*x;
		/* Cb=>G value is scaled-up -0.34414 * x */
		/* We also add in ONE_HALF so that need not do it in inner loop */
		cb_g_tab[i]=(-FIX(0.34414))*x+ONE_HALF;
	}

	range_limit=range_table+256;

	for(int i=0;i<256;i++)
	{
		range_table[i]=0;
		range_table[i+256]=i;
		range_table[i+512]=255;
	}
}

static void XeeYUVImageReadPixel(void *datarow,int x,uint8 *pixel,void *context)
{
	uint8 *row=(uint8 *)datarow;
	int y=row[x*2+1];
	int cb=row[(x&~1)*2];
	int cr=row[(x|1)*2];

	/* Range-limiting is essential due to noise introduced by DCT losses. */
	pixel[0]=range_limit[y+cr_r_tab[cr]];
	pixel[1]=range_limit[y+((cb_g_tab[cb]+cr_g_tab[cr])>>SCALEBITS)];
	pixel[2]=range_limit[y+cb_b_tab[cb]];
}

