...
-(SEL)identifyFile
{
	...
	if([header length]<12||read_be_uint32(headbytes)!='FORM'||read_be_uint32(headbytes+8)!='ILBM') return NULL;

	iff=[[XeeIFFHandle IFFHandleWithPath:filename fileType:'ILBM'] retain];
	if(!iff) return NULL;
	...
	switch([iff nextChunk])
	{
		...
		case 'DEST':
			NSLog(@"IFF warning: image has DEST chunk");
		break;
		...
		case 'BODY':
			...
			else if(planes==24) [self setDepthRGB:8];
...
-(SEL)load
{
	@try
	{
		if(planes<=8)
		{
			...
			if(masking==1) [self readRow:mask+current_line*rowbytes];
		}
		else if(planes==24)
		{
			...
			[self setCompletedRowCount:current_line+1];
		}
	}
	@catch(id exception)
	{
...
-(void)readRow:(uint8 *)row
{
	...
	{
		XeeUnPackBitsFromFile(iff,row,rowbytes,1);
	}
}
