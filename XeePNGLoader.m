...
static int is_png_gray_palette(png_structp png,png_infop info);
static void xee_png_error_func(png_structp png_ptr,png_const_charp error_msg);
static void xee_png_warning_func(png_structp png_ptr,png_const_charp warning_msg);
...
-(SEL)identifyFile
{
	...
	if([header length]<8||!png_check_sig((unsigned char *)[header bytes],8)) return NULL;

	png=png_create_read_struct(PNG_LIBPNG_VER_STRING,NULL,xee_png_error_func,xee_png_warning_func);
	if(!png) return NULL;
	...
	png_textp comments;
	first_comments=png_get_text(png,info,&comments,NULL);
	if(first_comments)
	{
		commentprops=[[NSMutableArray array] retain];
		for(int i=0;i<first_comments;i++)
		{
			[commentprops addObject:XeeNSStringFromByteBuffer(comments[i].key,strlen(comments[i].key))];
			[commentprops addObject:XeeNSStringFromByteBuffer(comments[i].text,comments[i].text_length)];
		} 
	}
...
-(SEL)finishLoading
{
	...
	png_textp comments;
	int all_comments=png_get_text(png,info,&comments,NULL);
	if(all_comments>first_comments)
	{
		if(!commentprops) commentprops=[[NSMutableArray array] retain];
		for(int i=first_comments;i<all_comments;i++)
		{
			[commentprops addObject:XeeNSStringFromByteBuffer(comments[i].key,strlen(comments[i].key))];
			[commentprops addObject:XeeNSStringFromByteBuffer(comments[i].text,comments[i].text_length)];
		}
	...
	if(commentprops)
	{
		[properties addObject:@"PNG comments"];
...
static void xee_png_error_func(png_structp png_ptr,png_const_charp error_msg)
{
	[NSException raise:@"XeeLibPNGException" format:@"libpng error: %s",error_msg];
}
